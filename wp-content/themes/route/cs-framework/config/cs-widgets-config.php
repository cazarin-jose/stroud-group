...
class CS_Shortcode_Widget extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    if ( !empty( $title ) ) { echo wp_kses( $before_title . $title . $after_title, 'post' ); }

    $content_widget = ( !empty( $instance['filter'] ) ) ? wpautop( $text ) : $text;
    echo '<div class="textwidget contactt">'. do_shortcode( $content_widget ) .'</div><div class="clear"></div>';
  ...
  ?>
    ...
    <p><a href="#" onmousedown="return false;" class="button button-primary shortcode-button" data-target="<?php echo esc_attr( $this->get_field_id('text') ); ?>">Quick Shortcode</a></p>
    <textarea class="widefat" rows="16" cols="20" id="<?php echo esc_attr( $this->get_field_id('text') ); ?>" name="<?php echo esc_attr( $this->get_field_name('text') ); ?>"><?php echo wp_kses( $text, 'post' ); ?></textarea>
    <p><input id="<?php echo esc_attr( $this->get_field_id('filter') ); ?>" name="<?php echo esc_attr( $this->get_field_name('filter') ); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo esc_attr( $this->get_field_id('filter') ); ?>">Automatically add paragraphs</label></p>
  <?php
  }
}
...
class CS_About_Widget extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    echo '<div class="textwidget contacts">';

    echo '<p>';
    echo ( ! empty( $instance['img'] ) ) ? '<img src="'. $instance['img'] .'" class="footer-logo-left" />' : '';
    echo wp_kses( $instance['logo_text'], 'post' );
    echo '</p>';
  ...
  function form( $instance ) {
    ...
    // WIDGET TITLE
    cs_get_field( array( 'id' => esc_attr( $this->get_field_name('title') ), 'name' => esc_attr( $this->get_field_name('title') ), 'type' => 'text', 'title' => 'Widget Title' ), $title );
    ...
    if( have_posts() ) :
      ...
      while( have_posts() ) : the_post();
        ...
        if( $instance['display_date'] ) {
          echo '<span class="post-date"><i class="fa fa-clock-o"></i> '. get_the_date() .'</span>';
        }

        if( $instance['display_category'] ) {
          echo '<span class="post-category"><i class="fa fa-folder-o"></i> '. $post_cats .'</span>';
        }
  ...
  function widget( $args, $instance ) {
    ...
    if( have_posts() ) :
      ...
      while( have_posts() ) : the_post();
        ...
        if( $instance['display_category'] ) {
          echo '<span class="post-category"><i class="fa fa-folder-o"></i> '. $item_slugs .'</span>';
        }
...
function custom_widgets_init(){
  call_user_func( 'register_widget', 'CS_Shortcode_Widget' );
  call_user_func( 'register_widget', 'CS_About_Widget' );
  call_user_func( 'register_widget', 'CS_Flickr_Widget' );
  call_user_func( 'register_widget', 'CS_Portfolio_Photos_Widget' );
  call_user_func( 'register_widget', 'CS_Blog_Posts_Widget' );
  call_user_func( 'register_widget', 'CS_Portfolio_Posts_Widget' );
  call_user_func( 'register_widget', 'CS_Side_Menu_Widget' );
}
