        ...
        switch ( $module['module'] ) {
          ...
          case 'wplogin':
            ...
            if ( is_user_logged_in() ) {
              $out  .= '<a href="'. wp_logout_url() .'"><i class="cs-in fa fa-power-off"></i>'. __( 'Log Out', 'route' ) . '</a>';
            } else {
              ...
              $out  .= '<div class="cs-top-modal">';
              $out  .= '<a href="#" class="cs-open-modal"><i class="cs-in fa fa-user"></i>'. __( 'Log In', 'route' ) . '</a>';
          ...
          case 'woologin':
            ...
            if( is_woocommerce_activated() ) {
              ...
              if ( is_user_logged_in() ) {
                $out  .= '<a href="'. get_permalink( wc_get_page_id( 'myaccount' ) ) .'"><i class="cs-in fa fa-user"></i>'. __( 'My Account Page', 'route' ) . '</a>';
                $out  .= ' / ';
                ...
                $login_form = str_replace( 'button-primary', cs_get_button_class( array( 'size' => 'xxs' ) ), $login_form );
                $out  .= '<div class="cs-top-modal">';
                $out  .= '<a href="#" class="cs-open-modal"><i class="cs-in fa fa-user"></i>'. __( 'Login', 'route' ) . '</a>';
                $out  .= '<div class="cs-modal-content">';
                $out  .= $login_form;
                $out  .= '<p class="cs-modal-lost-password">';
                $out  .= ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' ) ? '<a href="'. get_permalink( wc_get_page_id( 'myaccount' ) ) .'">'. __( 'Register', 'route' ) .'</a> - ' : '';
                $out  .= '<a href="'. wc_lostpassword_url() .'">'. __( 'Lost your password?', 'route' ) .'</a>';
                $out  .= '</p>';
                $out  .= '</div>';
                $out  .= '</div>';
              }
          ...
          case 'wooprice':
            ...
            if( is_woocommerce_activated() ) {
              $out  .= '<a href="'. wc_get_cart_url() .'"><i class="cs-in fa fa-shopping-cart"></i><span class="cs-cart-contents">'. sprintf( _n( '%d item', '%d items', WC()->cart->cart_contents_count, 'route' ), WC()->cart->cart_contents_count ) . WC()->cart->get_cart_total() .'</span></a>';
            }
...
if ( ! function_exists( 'cs_paging_nav' ) ) {
  function cs_paging_nav( $args = array() ) {
    ...
    if( $args['nav'] == 'load' ) {
      ...
      echo '<a href="#" class="ajax-load-more '. cs_get_button_class( array( 'size' => 'xxs' ) ) .'" data-token="'. $uniqid .'">';
      echo __( 'Load More', 'route' );
      echo '<span class="cs-loader"></span>';
      echo '</a>';
      echo '</div>';
...
if( ! function_exists( 'cs_portfolio_item' ) ) {
  function cs_portfolio_item( $item_args ) {
    ...
    if ( ! empty( $custom_layout ) ) {
      echo wp_kses( $custom_layout, 'post' );
      return;
    }
    ...
    $large_image_url_src     = ( ! empty( $large_image_url[0] ) ) ? $large_image_url[0] : '';
    $thumb_image_url         = wp_get_attachment_image_src( get_post_thumbnail_id(), $size );
    $thumb_image_url_src     = ( ! empty( $thumb_image_url[0] ) ) ? $thumb_image_url[0] : '';
    $lightbox_image          = ( ! empty( $post_options['custom_lightbox_link'] ) ) ? $post_options['custom_lightbox_link'] : $large_image_url_src;
    $lightbox_thumb          = ( ! empty( $post_options['custom_thumbnail'] ) ) ? $post_options['custom_thumbnail'] : $thumb_image_url_src;
