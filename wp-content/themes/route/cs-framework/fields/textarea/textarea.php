    ...
    if( isset( $this->field['multilang'] ) && ( is_wpml_activated() || is_qtranslate_activated() || is_polylang_activated() ) ) {
      ...
      foreach ( $languages as $key => $value ) {
        ...
        if( $is_shortcode ){
          ...
          echo '<p class="'. $class .'"><a href="#" onmousedown="return false;" class="button button-primary shortcode-button" data-target="'. $this->field['id'] .'-'. $uniqid .'">Quick Shortcode</a></p>';
        }
      ...
      if( $is_shortcode ){
        $uniqid  = uniqid();
        $elem_id = ' id="'. $this->field['id'] . '-'. $uniqid .'"';
        echo '<p><a href="#" onmousedown="return false;" class="button button-primary shortcode-button" data-target="'. $this->field['id'] .'-'. $uniqid .'">Quick Shortcode</a></p>';
      }
