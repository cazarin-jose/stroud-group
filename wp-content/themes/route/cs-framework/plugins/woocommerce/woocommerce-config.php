...
if ( ! function_exists( 'cs_woocommerce_top_bar_ajax') ) {
  function cs_woocommerce_top_bar_ajax( $fragments ) {
    ...
    $fragments['.cs-cart-contents'] = '<span class="cs-cart-contents">'. sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'route'), $woocommerce->cart->cart_contents_count) .' - '. $woocommerce->cart->get_cart_total() .'</span>';
...
if ( ! function_exists( 'cs_woocommerce_get_product_thumbnail' ) ) {
  function cs_woocommerce_get_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
    ...
    if ( has_post_thumbnail() ) {
      ...
      $output .= ( ! $product->is_in_stock() ) ? '<span class="out-of-stock">'. __( 'Out of stock', 'route' ) .'</span>' : '';
...
if ( ! function_exists( 'cs_woocommerce_order_again_button' ) ) {
  ...
  function cs_woocommerce_order_again_button( $order ) {
    ...
    ?>
    <p class="order-again">
      <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'order_again', $order->id ) , 'woocommerce-order_again' ) ); ?>" class="button <?php echo cs_get_button_class(); ?>"><i class="fa fa-refresh"></i> <?php _e( 'Order Again', 'route' ); ?></a>
    </p>
    <?php
  }
  ...
  function cs_get_product_search_form () {
    ...
    $form = '<div class="cs-search-form">
      <form action="' . esc_url( home_url( '/'  ) ) . '" method="get">
        <input type="text" value="' . get_search_query() . '" name="s" class="cs-search" placeholder="' . esc_attr_x( 'Search Products&hellip;', 'placeholder', 'route' ) . '" />
